...
const Navbar = ({ bodyRef }: NavbarProps) => {
  ...
  // Local state to track dark mode (for icon display)
  ...
  return (
    <div className="relative">
      ...
      <motion.nav 
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className="fixed top-0 w-full h-16 z-50 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md"
      >
        <div className="h-full px-4 flex items-center justify-between">
          <motion.div 
            className={`flex items-center gap-3 ${open ? "opacity-0" : "opacity-100"}`}
          ...
          >
            ...
            />
            <h1 className={`${bebas_neue.className} text-2xl tracking-wide text-green-600 dark:text-orange-500`}>
              Fin Track
            </h1>
          </motion.div>
            ...
            {!isLoggedIn ? (
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className={`${poppins.className} hover:bg-zinc-100 dark:hover:bg-zinc-800`}
                >
                  Login
                </Button>
                <Button
                  onClick={() => handleRegisterLogin("Register")}
                  className={`${poppins.className} bg-green-500 hover:bg-green-600 dark:bg-orange-500 dark:hover:bg-orange-600`}
                >
                  Register
                </Button>
              </div>
                ...
                <SidebarContent className="px-2">
                  <div className="space-y-1">
                    {items.map((item, index) => (
                      <Link href={item.url} key={index} onClick={() => setOpen(false)}>
                        <motion.div
                        ...
                        >
                          ...
                          />
                          <span className={`${poppins.className} ${selectedPath(item.url) ? "font-medium" : ""}`}>
                            {item.title}
                          </span>
                        </motion.div>
                      </Link>
